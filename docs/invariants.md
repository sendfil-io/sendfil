...
Current repo note: the primary live App path still estimates and submits through the EVM/wagmi `useExecuteBatch` flow for EVM senders. `src/lib/transaction/nativeBatchMessage.ts` can prepare the single native Filecoin `InvokeEVM` message for a native `f1`/`t1` sender from the same prepared Multicall3 batch payload, and `src/lib/transaction/nativeBatchPreflight.ts` can fetch the native nonce and Lotus gas estimate for that message. A FilSnap-backed native Calibration `t1` test path is available only when `VITE_NATIVE_FILECOIN_WALLET_ENABLED=true` and `VITE_NATIVE_FILECOIN_TESTNET_SEND_ENABLED=true`; it signs with `fil_signMessage`, submits with `fil_sendMessage`, and remains limited to Calibration testing. A Ledger-backed native Calibration `t1` test path is available only when `VITE_NATIVE_FILECOIN_WALLET_ENABLED=true` and `VITE_LEDGER_FILECOIN_TESTNET_SEND_ENABLED=true`; it uses WebHID, derives `m/44'/1'/0'/0/0`, signs the Zondax-serialized native batch message on device, and submits the signed secp256k1 message through Lotus. CI verifies the provider boundary and submission shape, but not real hardware behavior. Native mainnet `f1` sending and generic native wallet support remain planned, not live.
