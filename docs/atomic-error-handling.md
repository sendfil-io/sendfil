...
- `PARTIAL` remains the default mode in `App.tsx`.
- `ATOMIC` is selectable in the live app flow on this branch.
...
Both modes still encode through `aggregate3Value(...)`. There is no separate `aggregate3(...)` encoding path in this repo today. The all-or-revert guarantee comes from `allowFailure=false` on every call plus the live UI wiring that propagates `errorMode` from configuration through preflight and send.
...
The code does not currently ship a production feature flag for ATOMIC mode. Operational rollback would mean restoring the ATOMIC selector guard in `App.tsx`, while leaving the lower-level batch builder support intact.
